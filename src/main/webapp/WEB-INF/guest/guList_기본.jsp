		...
		<table class="table table-borderless mb-0">
			<tr>
				<td>
					...
					<c:if test="${sAMid == 'admin'}">
						<a href="${ctp}/adminLogout.gu" class="btn btn-sm btn-secondary">관리자 로그아웃</a>
					</c:if>
				</td>
				<td style="text-align:right;"><a href="${ctp}/guest/guInput.gu" class="btn btn-sm btn-secondary">글쓰기</a></td>
			</tr>
		</table>
		<c:set var="no" value="${fn:length(vos)}"></c:set>
	 <c:forEach var="vo" items="${vos}" varStatus="st"> 
		...
		<table class="table table-bordered">
			...
			<tr>
				<th>전자우편</th>
				<td colspan="3">
					<c:if test="${fn:length(vo.email)<=5||fn:contains(vo.email,'@')==false}">- 없음 -</c:if>
					<c:if test="${fn:length(vo.email)>5&&fn:contains(vo.email,'@')}"><a href="${vo.email}" target="_blank">${vo.email}</a></c:if>
				</td>
			</tr>
			<tr>
				<th>홈페이지</th>
