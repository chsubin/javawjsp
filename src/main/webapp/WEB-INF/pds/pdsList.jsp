...
<div class="container">
  ...
  <table class="table table-borderless">
    <tr>
      ...
      <td class="text-right">
        <a href="${ctp}/pdsInput.pds?part=${part}&pag=${pag}" class="btn btn-success">자료올리기</a>
      </td>
    </tr>
  </table>
  <table class="table table-hover text-center">
    ...
    <c:forEach var="vo" items="${vos}">
      <tr>
        ...
        <td>
          ...
          <c:if test="${vo.fSize/1024<1000}">(<fmt:formatNumber value="${vo.fSize/1024}" pattern="#,###" />KByte)</c:if>
          <c:if test="${vo.fSize/1024>=1000}">(<fmt:formatNumber value="${vo.fSize/1024/1024}" pattern="#,###" />MByte)</c:if>
        </td>
        <td>${vo.downNum}</td>
        <td>
          <a href="#" onclick="modalView('${vo.title}','${vo.nickName}','${vo.mid}','${vo.part}','${vo.fName}','${vo.fSName}','${vo.fSize}','${vo.downNum}','${vo.fDate}')" class="badge badge-primary badge-sm" data-toggle="modal" data-target="#myModal">모달창</a><br/>
          <a href="${ctp}/pdsTotalDown.pds?idx=${vo.idx}" class="badge badge-secondary mb-1">전체다운</a><br/>
    ...
    <tr>
    	<td colspan="8" class="text-center">
	    	<form name="mySearchForm" method="post" action="${ctp}/pdsSearch.pds">
		    	<select name="searchfield">
		    		<option value="title" ${searchfield == 'title' ? 'selected' : ''}>제목</option>
		    		<option value="nickName" ${searchfield == 'nickName' ? 'selected' : ''}>올린이</option>
...
<div class="text-center">
  <ul class="pagination justify-content-center">
    <c:if test="${pag > 1}">
      <li class="page-item"><a class="page-link text-secondary" href="${ctp}/pdsList.pds?part=${part}&pag=1&searchfield=${searchfield}&search=${search}">첫페이지</a></li>
    </c:if>
    <c:if test="${curBlock > 0}">
      <li class="page-item"><a class="page-link text-secondary" href="${ctp}/pdsList.pds?part=${part}&pag=${(curBlock-1)*blockSize + 1}&searchfield=${searchfield}&search=${search}">이전블록</a></li>
    </c:if>
    <c:forEach var="i" begin="${(curBlock)*blockSize + 1}" end="${(curBlock)*blockSize + blockSize}" varStatus="st">
      <c:if test="${i <= totPage && i == pag}">
    		<li class="page-item active"><a class="page-link bg-secondary border-secondary" href="${ctp}/pdsList.pds?part=${part}&pag=${i}&searchfield=${searchfield}&search=${search}">${i}</a></li>
    	</c:if>
      <c:if test="${i <= totPage && i != pag}">
    		<li class="page-item"><a class="page-link text-secondary" href="${ctp}/pdsList.pds?part=${part}&pag=${i}&searchfield=${searchfield}&search=${search}">${i}</a></li>
    	</c:if>
    </c:forEach>
    <c:if test="${curBlock < lastBlock}">
      <li class="page-item"><a class="page-link text-secondary" href="${ctp}/pdsList.pds?part=${part}&pag=${(curBlock+1)*blockSize + 1}&searchfield=${searchfield}&search=${search}">다음블록</a></li>
    </c:if>
    <c:if test="${pag < totPage}">
      <li class="page-item"><a class="page-link text-secondary" href="${ctp}/pdsList.pds?part=${part}&pag=${totPage}&searchfield=${searchfield}&search=${search}">마지막페이지</a></li>
    </c:if>
  </ul>
</div>
<!-- 블록 페이지 끝 -->
