...
public class PdsDAO {
  ...
  // 파트별 전체 자료건수 가져오기
	public int totRecCnt(String part, String searchfield, String search) {
		int totRecCnt = 0;
		try {
			if(!search.equals("")) {
				if(part.equals("전체")) {
					...
					sql = "select count(*) as cnt from pds where part = ? and "+searchfield+" like ? ";
					pstmt = conn.prepareStatement(sql);
	...
	// 파트별 전체 자료 가져오기
	public ArrayList<PdsVO> getPdsList(int startIndexNo, int pageSize, String part, String searchfield, String search) {
		ArrayList<PdsVO> vos = new ArrayList<>();
		System.out.println(searchfield);
		System.out.println(search);
		try {
			if(!search.equals("")&&part.equals("전체")) {
			...
			else if(!search.equals("")&&!part.equals("전체")) {
				sql = "SELECT 	*,datediff(now(), fDate) as day_diff, "
						+ "timestampdiff(hour, fDate, now()) as hour_diff "
						+ "FROM pds where part=? and "+searchfield+" like ? order by idx desc "
						+ "limit ?,?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, part);
				pstmt.setString(2, "%"+search+"%");
