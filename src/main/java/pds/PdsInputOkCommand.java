...
public class PdsInputOkCommand implements PdsInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		//업로드시킨 멀티파일에 대한 처리(올린파일명과 서버에 저장된 파일명을 '/'로 결합해서 준비한다.
		...
		while(fileNames.hasMoreElements()) {
			...
			if(multipartRequest.getFilesystemName(file) !=null) {
				originalFileName += multipartRequest.getOriginalFileName(file)+"/"; //원래파일명
				filesystemName += multipartRequest.getFilesystemName(file)+"/"; //저장되는 파일명
			}
		}
		originalFileName.substring(0, originalFileName.length()-1);
		filesystemName.substring(0, filesystemName.length()-1);
		...
		//전송 폼의 값들을 모두 받아오기
		String listPart = multipartRequest.getParameter("listPart")==null?"학습":multipartRequest.getParameter("listPart");
